...
func addVariables(dcs []*DeployConfig, idx int, vars []Var) error {
	...
	for _, v := range vars {
		...
		err := dc.GetVariables().Register(Variable{
			Name:  v.name,
			Value: getValue(v.name, dcs, idx),
		})
		if err != nil {
			return errors.New("failed to register variable to deploy config")
		}
	}
...
func getValue(name string, dcs []*DeployConfig, idx int) string {
	...
	case "service_addr":
		return dc.GetHostIp()
	case "service_port":
		return utils.Atoa(dc.GetListenPort())
	case "service_client_port": // etcd
		return utils.Atoa(dc.GetListenClientPort())
	case "service_dummy_port": // mds, snapshotclone
		return utils.Atoa(dc.GetListenDummyPort())
	case "service_proxy_port": // snapshotclone
		return utils.Atoa(dc.GetListenProxyPort())
	case "service_external_addr": // chunkserver, metaserver
		return utils.Atoa(dc.GetListenExternalIp())
	case "service_external_port": // metaserver
		if utils.Atoa(dc.GetEnableExternalServer()) == "true" {
			return utils.Atoa(dc.GetListenExternalPort())
		}
		return utils.Atoa(dc.GetListenPort())
	case "log_dir":
		return dc.GetLogDir()
